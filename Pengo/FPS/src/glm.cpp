/*
	  ...
	  Lynek - May 2014 - remotion of some bugs in the texture support
                       - added function glmDrawClip();
*/
...
GLvoid glmDraw(GLMmodel* model, GLuint mode,char *drawonly)
{
    ...
    if (mode & GLM_TEXTURE) {
        ...
        glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); // old GL_MODULATE -> multiplies the glColor to the texture
    }
...
// Draws only a part of a model in the positive side given a plane equation
GLvoid glmDrawClip(GLMmodel* model, GLuint mode, double* planeEquation)
{
    static GLuint i;
    static GLMgroup* group;
    static GLMtriangle* triangle;
    ...
    while (group)
	{
        ...
        for (i = 0; i < group->numtriangles; i++) {
			...
			if (!triangle->visible) continue;
#endif
            if(model->vertices[3 * triangle->vindices[0]+0]*planeEquation[0] + model->vertices[3 * triangle->vindices[0]+1]*planeEquation[1] + model->vertices[3 * triangle->vindices[0]+2]*planeEquation[2] + planeEquation[3] < 0 &&
               model->vertices[3 * triangle->vindices[1]+0]*planeEquation[0] + model->vertices[3 * triangle->vindices[1]+1]*planeEquation[1] + model->vertices[3 * triangle->vindices[1]+2]*planeEquation[2] + planeEquation[3] < 0 &&
               model->vertices[3 * triangle->vindices[2]+0]*planeEquation[0] + model->vertices[3 * triangle->vindices[2]+1]*planeEquation[1] + model->vertices[3 * triangle->vindices[2]+2]*planeEquation[2] + planeEquation[3] < 0)
            {
                if (mode & GLM_FLAT)
                glNormal3fv(&model->facetnorms[3 * triangle->findex]);
